...
const parseParameter = ({ name, type, loc }) => (ctx) => {
    return ctx.ast.TypedParameter(parseOptionalId(name)(ctx), parseType(type)(ctx), loc);
};
...
const parseTypeOptional = ({ type, loc }) => (ctx) => {
    ...
    if (name.$ === "MapKeyword") {
        ...
        // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- eslint bug
        valueAs ? ctx.ast.Id(valueAs.name, valueAs.loc) : undefined, genericLoc);
    }
    if (name.$ === "Bounced") {
        const parsedArgs = parseList(args);
...
const parseAsmFunction = (node) => (ctx) => {
    return ctx.ast.AsmFunctionDef(parseAsmShuffle(node.shuffle)(ctx), parseFunctionAttributes(node.attributes, false, node.loc)(ctx), parseId(node.name)(ctx), node.returnType ? parseType(node.returnType)(ctx) : undefined, map(parseList(node.parameters), parseParameter)(ctx), [node.instructions.trim()], node.loc);
};
...
const parseNativeFunctionDecl = ({ name, attributes, nativeName, parameters, returnType, loc, }) => (ctx) => {
    return ctx.ast.NativeFunctionDecl(map(attributes, parseFunctionAttribute)(ctx), parseId(name)(ctx), parseFuncId(nativeName)(ctx), map(parseList(parameters), parseParameter)(ctx), returnType ? parseType(returnType)(ctx) : undefined, loc);
};
