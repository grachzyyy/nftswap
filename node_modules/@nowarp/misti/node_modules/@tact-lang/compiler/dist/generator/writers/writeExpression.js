    ...
    if (structType.kind === "ref") {
        ...
        if (type.kind === "struct" || type.kind === "contract") {
            if (isStructInThenBranch) {
                return `(${writeExpression(condition, wCtx)} ? ${ops_1.ops.typeAsOptional(type.name, wCtx)}(${writeExpression(structExpr, wCtx)}) : null())`;
            }
            else {
                return `(${writeExpression(condition, wCtx)} ? null() : ${ops_1.ops.typeAsOptional(type.name, wCtx)}(${writeExpression(structExpr, wCtx)}))`;
            }
        }
    }
    return "";
