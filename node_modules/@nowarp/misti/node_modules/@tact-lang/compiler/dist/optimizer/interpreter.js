...
source, util) {
    switch (op) {
        ...
        case "/": {
            ...
            const astLeft = ensureInt(valLeft);
            const result = (0, util_3.divFloor)(astLeft.value, astRight.value);
            return ensureInt(util.makeNumberLiteral(result, source));
        }
        case "%": {
            ...
            if (astRight.value === 0n)
                throwErrorConstEval("divisor expression must be non-zero", astRight.loc);
            const astLeft = ensureInt(valLeft);
            const result = (0, util_3.modFloor)(astLeft.value, astRight.value);
            return ensureInt(util.makeNumberLiteral(result, source));
        }
        case "&": {
            const astLeft = ensureInt(valLeft);
...
function parseAndEvalExpression(sourceCode, ast = (0, ast_helpers_1.getAstFactory)(), parser = (0, grammar_1.getParser)(ast), util = (0, util_2.getAstUtil)(ast)) {
    try {
...
class Interpreter {
    ...
    evalStaticFunction(functionCode, args, returns) {
        ...
        const paramNames = functionCode.params.flatMap((param) => param.name.kind === "id" ? [(0, ast_helpers_1.idText)(param.name)] : []);
    ...
    interpretLetStatement(ast) {
        ...
        const id = (0, ast_helpers_1.idText)(ast.name);
        if ((0, resolveDescriptors_1.hasStaticConstant)(this.context, id)) {
            // Attempt of shadowing a constant in a let declaration
            (0, errors_1.throwInternalCompilerError)(`declaration of ${id} shadows a constant with the same name`, ast.loc);
        }
    ...
    interpretDestructStatement(ast) {
        for (const [_, name] of ast.identifiers.values()) {
            ...
            const id = (0, ast_helpers_1.idText)(name);
            if ((0, resolveDescriptors_1.hasStaticConstant)(this.context, id)) {
    ...
    interpretAugmentedAssignStatement(ast) {
        if (ast.path.kind === "id") {
            ...
            const newVal = evalBinaryOp((0, util_1.binaryOperationFromAugmentedAssignOperation)(ast.op), currentPathValue, updateVal, ast.loc, this.util);
