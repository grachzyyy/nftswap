...
async function build(args) {
    ...
    const parser = args.parser ?? (0, grammar_1.getParser)(ast);
    ...
    for (const contract of sortedContracts ?? allContracts) {
        ...
        const pathCodeFif = project.resolve(config.output, `${config.name}_${contractName}.fif`);
    ...
    for (const pkg of packages) {
        ...
        try {
            const bindingsServer = (0, writeTypescript_1.writeTypescript)(JSON.parse(pkg.abi), ctx, built[pkg.name]?.constants ?? [], built[pkg.name]?.contract, {
                code: pkg.code,
