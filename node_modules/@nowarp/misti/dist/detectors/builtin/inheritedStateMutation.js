...
exports.InheritedStateMutation = void 0;
 ...
 * ```
 */
class InheritedStateMutation extends detector_1.AstDetector {
    ...
    async check(cu) {
        return Array.from(cu.ast.getContracts()).reduce((acc, contract) => {
            ...
            if (!contractAst)
                throw exceptions_1.InternalException.make(`Cannot find contract AST: ${contract.name}`);
            const inheritedFields = cu.ast.getInheritedFields(contract.id);
            if (inheritedFields === undefined) {
                throw exceptions_1.InternalException.make(`Cannot fetch trait fields for contract ${contract.name}`);
            }
            const inheritedFieldNames = inheritedFields.map((f) => f.name.text);
            const check = (node) => (0, tact_1.foldStatements)(node, (acc, stmt) => {
                return this.findInheritedFieldAssignments(acc, stmt, inheritedFieldNames);
            }, []);
    ...
    findInheritedFieldAssignments(acc, stmt, inheritedFieldNames) {
        const mutations = (0, tact_1.collectMutations)(stmt);
        const foundMutations = mutations
            ? (0, util_1.intersectLists)(inheritedFieldNames, (0, tact_1.mutationNames)(mutations.mutatedFields))
            : [];
        if (foundMutations.length > 0) {
            const mut = foundMutations.join(", ");
            acc.push(this.makeWarning(`Inherited trait field${foundMutations.length === 1 ? ` ${mut} is` : `s ${mut} are`} mutated`, stmt.loc, {
...
exports.InheritedStateMutation = InheritedStateMutation;
